    ...
    login(){
        ...
        if(this.state.phone.length == 11&&this.state.code){
            this.props.axios({
                data:JSON.stringify({
                    query:`mutation{
                    login:login(phone:"${this.state.phone}",code:"${this.state.code}"){
                    status
                    msg
                    user{
                    id
                ...
                if(res.data.login.status==0){
                    ...
                    this.props.setCookie('token',user.token);
                    //this.props.dispatch(saveUser(user))
                    this.props.login(user.token)
                }else {
                    this.props.alert(res.data.login.msg)
                }
            })
        }
        else if(this.state.phone.length != 11){
            this.props.alert('请输入正确的手机号');
        }else if(!this.state.code){
            this.props.alert('请输入验证码');
        }
    }
    QQLogin(){
        let redirect = encodeURIComponent(`http://${window.location.host}/login?url=`+encodeURIComponent(window.location.href));
...
export default Request(NoLogin);